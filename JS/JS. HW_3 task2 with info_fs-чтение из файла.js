...
// В этом коде подробно расписаны действия и выводится в коноль вся логика решения
...
function person (person) {
    console.log('количесвто элментов на входе =', person.length) // просто для инфы
    for (i = 0; i < person.length - 1; i++) {               // делаю цикл, который будет перебирать элементы и одновременно сравнивать
        for (i2 = (person.length - 1); i2 > i; i2--) {      // вложенный цикл для встречного перебора элементов в массиве с конца
            console.log('Сравниваю', person[i].name, 'с номером', i, 'с номером', i2, person[i2].name)      // подробно расписываю для самопроверки какие имена сравниваются
            if (person[i].username == person[i2].username && person[i].email == person[i2].email) {         // сравниваю 'username' и 'email', если они равны, то удаляю элемент, который ближе к концу списка
                console.log('они равны - удаляем элемент под номером', i2, '\n')
                person.splice(i2,1)
            }
        }
    }

    console.log('\n', 'Итоговая длина массива =',person.length, '\n')  // просто для инфы
    for (k1 = 0; k1 < person.length; k1++) {               // выожу для визуальной проверки итоговый список людей
...
// ниже JSON с которым отдельно работал
/* person ([{
    "name": "Leanne Graham",
    "username": "Bret",
